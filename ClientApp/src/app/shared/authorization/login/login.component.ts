...
export class LoginComponent implements OnInit {
  ...
  private redirectToRegister(role?): any {
    const roleParam = role ? `&role=${role}` : '';
    this.redirectToApiAuthorizationPath(
      `${ApplicationPaths.IdentityRegisterPath}?returnUrl=${encodeURI('/' + ApplicationPaths.Login)}${roleParam}`);
  }
