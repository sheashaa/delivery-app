...
export class LogoutComponent implements OnInit {
    ...
    switch (action.path) {
      case LogoutActions.Logout:
        if (!!window.history.state.local) {
          await this.logout(undefined);
          //await this.logout(this.getReturnUrl());
        } else {
          // This prevents regular links to <app>/authentication/logout from triggering a logout
          //this.message.next('The logout was not initiated from within the page.');
          console.error('The logout was not initiated from within the page.');
        }
  ...
  private async logout(returnUrl: string): Promise<void> {
    ...
    if (isauthenticated) {
      const result = await this.authorizeService.signOut(undefined);
  ...
  private async processLogoutCallback(): Promise<void> {
    ...
    const result = await this.authorizeService.completeSignOut(undefined);
